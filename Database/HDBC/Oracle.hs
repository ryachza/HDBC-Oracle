...
import Database.HDBC.Oracle.OCIFunctions (
		EnvHandle, ErrorHandle, ConnHandle,
...
data OracleStatement = OracleStatement {
	state :: StmtState,
	handle :: StmtHandle
}
...
rethrowOCI errorHandle action = catchOCI action (\exc -> do
		(code, msg) <- getOCIErrorMsg (castPtr errorHandle) oci_HTYPE_ERROR
#if __GLASGOW_HASKELL__ >= 610
...
prepareOracle oraconn@(OracleConnection env err conn) query = rethrowOCI err (do
...
dtypeConversion = [
		(
			[oci_SQLT_CHR, oci_SQLT_AFC, oci_SQLT_AVC, oci_SQLT_LNG, oci_SQLT_RDD],
			(oci_SQLT_STR, 16000, readString)
		), (
			[oci_SQLT_DAT, oci_SQLT_DATE],
			...
			[oci_SQLT_BIN],
			(oci_SQLT_BIN, 2000, readBinary)
		)
	]

fetchOracleRow :: OracleConnection -> MVar OracleStatement -> IO (Maybe [SqlValue])
fetchOracleRow (OracleConnection _ err _) stmtvar = do
	result <- withMVar stmtvar fetch
	if isNothing result then finishOracle stmtvar else return ()
	return result
	where
		fetch (OracleStatement Prepared _) = fail "Trying to fetch before executing statement"
		fetch (OracleStatement (Executed convinfos) stmt) = do
			let readCols = map fst convinfos
			fr <- stmtFetch err stmt
			if fr == oci_NO_DATA then return Nothing else return . Just =<< sequence readCols

readString = readValue
	(\(_, buf, nullptr, sizeptr) -> bufferToString (undefined, buf, nullptr, sizeptr))
	SqlString

readTime = readValue
	(\(_, buf, nullptr, sizeptr) -> bufferToCaltime nullptr buf)
	(\time -> let TOD secs _ = toClockTime time in SqlEpochTime secs)

readNumber = readValue
	(\(_, buf, nullptr, sizeptr) -> bufferToString (undefined, buf, nullptr, sizeptr))
	strToSqlNum

readBinary = readValue
	(\(_, buf, nullptr, sizeptr) -> bufferToByteString buf nullptr sizeptr)
	SqlByteString

readValue :: (ColumnInfo -> IO (Maybe a)) -> (a -> SqlValue) -> ColumnInfo -> IO SqlValue
readValue read convert colinfo = read colinfo >>= return . maybe SqlNull convert

finishOracle :: MVar OracleStatement -> IO ()
finishOracle = flip modifyMVar_ (\stmt ->
		let
			stmthandle = handle stmt
		in do
			free stmthandle
...
getOracleColumnNames (OracleConnection _ err _) stmtvar = withMVar stmtvar getNames
	where
		getNames (OracleStatement Prepared _) = fail "Trying to read column names before executing"
